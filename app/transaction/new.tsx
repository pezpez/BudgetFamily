...
export default function NewTransactionScreen() {
  ...
  const selectedSub = categories
    .flatMap((c) => c.subcategories.map((s) => ({ ...s, categoryColor: c.color, categoryName: c.name })))
    ...
    <View style={[styles.root, { backgroundColor: theme.colors.background }]}>
      ...
      <Surface style={styles.amountCard} elevation={1}>
        <TextInput
          ...
          right={<TextInput.Affix text="€" textStyle={{ fontSize: 28, color: typeColor }} />}
        />
      </Surface>

      <ScrollView style={styles.form} contentContainerStyle={styles.formContent} keyboardShouldPersistTaps="handled">
        {/* Subcategory picker */}
        <Text variant="labelLarge" style={styles.label}>Catégorie</Text>
        {categories.map((cat) => (
          <View key={cat.id}>
        ...
        {/* Date */}
        <Text variant="labelLarge" style={[styles.label, { marginTop: 16 }]}>Date</Text>
        <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  amountInput: { backgroundColor: 'transparent', textAlign: 'center' },
  form: { flex: 1 },
  ...
  noteInput: { backgroundColor: '#F9FAFB', borderRadius: 12 },
